    ...
    : mValueTreeState(vts), tempoDisplay(vts), tempoKnob([](double v){return String(v);})
{
	...
	tempoSyncBtnAttachment = std::make_unique<AudioProcessorValueTreeState::ButtonAttachment>(mValueTreeState, "tempoSync", tempoSyncBtn);
	divComboBoxAttachment = std::make_unique<AudioProcessorValueTreeState::ComboBoxAttachment>(mValueTreeState, "division", divComboBox);
