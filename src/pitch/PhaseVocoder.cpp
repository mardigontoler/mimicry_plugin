    ...
    if (fifosWritten == PV::FFT_SIZE)
    {
        ...
        if(!juce::approximatelyEqual(factor, 1.0f)) { // pitch shift
            ...
            for (size_t i = 0; i < PV::FFT_SIZE; i++) {
                ...
                outputData[(static_cast<size_t>(floor(outputIndex)) + i) % outputData.size()] +=
            ...
            for (size_t i = 0; i < analysisHopSize; i++) {
                outputData[(static_cast<size_t>(floor(outputIndex)) + i) % outputData.size() ] = fifo[(fifoRead + i) % PV::FFT_SIZE];
            }
        }
...
float PhaseVocoder::nextSample() {
    if (outputReady)
    {
        ...
        const size_t rightIndex = (leftIndex + 1) % outputData.size();
...
[[maybe_unused]] size_t PhaseVocoder::getDelay() const {
